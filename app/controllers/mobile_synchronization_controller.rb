  ...
  def customers
    ...
    if @manager_id
      ...
      if params[:updated_at]
        # @customers = ShippingAddress.where(id: @shipping_address_ids).joins(:manager_shipping_addresses, :customer).uniq.select("customers.id, customers.address, customers.name, customers.external_key, customers.updated_at, customers.created_at, customers.debt, customers.validity").where("customers.updated_at >= ? OR manager_shipping_addresses.updated_at >= ?", params[:updated_at], params[:updated_at]).page(page).per(page_size) 
        @customers = Customer.where(id: @manager_customer_ids).where("updated_at >= ?", params[:updated_at]).page(page).per(page_size)
      else
        # @customers = ShippingAddress.where(id: @shipping_address_ids).joins(:manager_shipping_addresses, :customer).uniq.select("customers.id, customers.address, customers.name, customers.external_key, customers.updated_at, customers.created_at, customers.debt, customes.validity").page(page).per(page_size)
